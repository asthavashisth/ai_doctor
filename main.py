...
# Helper function to fetch recommendation data
def helper(dis):
...
def get_predicted_value(patient_symptoms):
    ...
    for item in patient_symptoms:
        cleaned_symptom = item.strip().lower().replace(" ", "_")
        if cleaned_symptom in symptoms_dict:
            input_vector[symptoms_dict[cleaned_symptom]] = 1
        else:
            invalid_symptoms.append(item)

    prediction_index = svc.predict([input_vector])[0]
    predicted_disease = diseases_list.get(prediction_index, "Disease Not Found")

    return predicted_disease, invalid_symptoms
...
def home():
    if request.method == 'POST':
        ...
        if not predicted_disease or predicted_disease == "Disease Not Found":
            message = "No valid symptoms entered. Please check spelling or refer to the symptom list."
            return render_template('index.html', message=message)
        ...
        return render_template(
            ...
            my_diet=rec_diet,
            workout=workout,
            message=f"Ignored invalid symptoms: {', '.join(invalid_symptoms)}" if invalid_symptoms else None
        )
